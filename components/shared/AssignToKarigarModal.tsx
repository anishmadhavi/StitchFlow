 ...
 * STATUS: FIXED (Large Image Grid Layout for Selection) ✅
 */
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {/* ✅ STEP 1: NEW GRID LAYOUT WITH LARGE IMAGES */}
        {step === 1 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-[65vh] overflow-y-auto p-1">
            {karigars.length === 0 ? (
              <p className="col-span-full text-center text-gray-500 py-4">No Karigars found.</p>
            ) : (
              karigars.map(k => {
                // Dual mapping for image source
                ...
                return (
                  <div 
                    ...
                    // Vertical card, no internal padding on the container to let image go full width
                    className="bg-white border-2 border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:border-blue-500 transition-all cursor-pointer group active:scale-95 flex flex-col"
                  >
                    {/* LARGE IMAGE CONTAINER - Aspect Square ensures it's a big box */}
                    ...
                    <div className="p-4 text-center bg-white relative z-10">
                      <h4 className="text-lg font-black text-gray-900 leading-tight group-hover:text-blue-600 transition-colors truncate">
                        {k.name}
                      </h4>
                    </div>
                  </div>
        ...
        {/* STEP 2: QUANTITY INPUT (Kept mostly the same, just updated preview image to be square too) */}
        {step === 2 && selectedKarigar && (
          <div className="space-y-8">
            ...
            <div className="flex items-center gap-4 bg-gray-50 p-3 rounded-2xl border border-gray-100">
              ...
              {/* Updated preview to be a square rounded corner image */}
              <div className="w-16 h-16 rounded-xl overflow-hidden border-2 border-white shadow-sm bg-gray-200 shrink-0">
              ...
              <div className="overflow-hidden">
                ...
                <h4 className="text-xl font-black text-gray-900 leading-none truncate">{selectedKarigar.name}</h4>
              </div>
            </div>

            {/* Quantity Inputs */}
            <div className="grid grid-cols-2 gap-x-4 gap-y-8 max-h-[40vh] overflow-y-auto pr-1 pb-2">
              {availableEntries.map(([size, max]) => (
                <div key={size} className="space-y-2">
                  <label className="block text-lg font-black text-gray-800 text-center uppercase tracking-tight">
                    {size} <span className="text-blue-600 text-sm ml-1">(Max: {max as number})</span>
                  </label>
                  <input 
                    type="number" 
                    min="0" 
                    ...
                    placeholder="0"
                    value={formQty[size] || ''} 
                    onChange={e => (e.target.value === '' || /^\d+$/.test(e.target.value)) && setFormQty({
                      ...formQty, 
            ...
            <button 
              ...
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-5 rounded-2xl text-xl shadow-xl shadow-blue-200 transition-all active:scale-95 flex items-center justify-center gap-2 uppercase tracking-widest disabled:opacity-50 disabled:active:scale-100 mt-2"
            >
