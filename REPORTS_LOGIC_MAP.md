### 🧠 Detailed Code Map (File-Wise Summary)

#### 📄 vite.config.ts
**Purpose:** Manages data fetching and external API communications.

**Key Logic/Functions:**
> 

#### 📄 types.ts
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> 

#### 📄 App.tsx
**Purpose:** Handles user authentication and security flows.

**Key Logic/Functions:**
> App, directly

#### 📄 batchService.ts
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> 

#### 📄 assignmentService.ts
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> 

#### 📄 userService.ts
**Purpose:** Handles user authentication and security flows.

**Key Logic/Functions:**
> 

#### 📄 Login.tsx
**Purpose:** Handles user authentication and security flows.

**Key Logic/Functions:**
> handleSubmit

#### 📄 ManagerDashboard.tsx
**Purpose:** Main administrative interface for managing data and items.

**Key Logic/Functions:**
> handleOpenQCModal, activeBatches, handleOpenAssignModal, pendingQCItems, handleOpenCutModal, pendingCuttingBatches

#### 📄 AdminDashboard.tsx
**Purpose:** Main administrative interface for managing data and items.

**Key Logic/Functions:**
> getActiveAssignments, staffUsers, handleOpenBatchDetails, activeBatches, handleOpenAssignModal, handleOpenPassbook

#### 📄 MasterDashboard.tsx
**Purpose:** Main administrative interface for managing data and items.

**Key Logic/Functions:**
> pendingBatches, handleOpenAssignModal, assignableBatches, calculateTotalAvailable, handleOpenCutModal

#### 📄 KarigarDashboard.tsx
**Purpose:** Main administrative interface for managing data and items.

**Key Logic/Functions:**
> calculateTotalQty, isAdvance, handlePhotoUpload

#### 📄 QCDashboard.tsx
**Purpose:** Main administrative interface for managing data and items.

**Key Logic/Functions:**
> pendingQCItems, formatDate, handleOpenInspectModal

#### 📄 BatchesTab.tsx
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> 

#### 📄 SettingsTab.tsx
**Purpose:** Manages data fetching and external API communications.

**Key Logic/Functions:**
> testConnection, handleSave, loadSettings

#### 📄 PassbookModal.tsx
**Purpose:** General Component/Utility

**Key Logic/Functions:**
> handleSubmit

#### 📄 PaymentsTab.tsx
**Purpose:** General Component/Utility

**Key Logic/Functions:**
> isAdvance

#### 📄 BatchDetailsModal.tsx
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> planned, cut

#### 📄 CreateUserModal.tsx
**Purpose:** Handles user authentication and security flows.

**Key Logic/Functions:**
> called, handleForceSubmit

#### 📄 StaffTab.tsx
**Purpose:** Handles user authentication and security flows.

**Key Logic/Functions:**
> 

#### 📄 QCTab.tsx
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> 

#### 📄 AllocationTab.tsx
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> totalAvailable

#### 📄 ProductionTab.tsx
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> 

#### 📄 CreateBatchModal.tsx
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> handleSubmit, handleShopifySync, handleImageUpload

#### 📄 CuttingModal.tsx
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> handleSubmit

#### 📄 QCInspectionModal.tsx
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> handleSubmit, handleQtyChange

#### 📄 AssignToKarigarModal.tsx
**Purpose:** Production logic for garment manufacturing and assignment.

**Key Logic/Functions:**
> handleSubmit

#### 📄 useData.ts
**Purpose:** Manages data fetching and external API communications.

**Key Logic/Functions:**
> fetchData, useData

#### 📄 useAuth.ts
**Purpose:** Handles user authentication and security flows.

**Key Logic/Functions:**
> handleLogin, handleLogout, handleSignUp, initializeAuth, loadProfile, useAuth

#### 📄 supabaseClient.ts
**Purpose:** Handles user authentication and security flows.

**Key Logic/Functions:**
> 
