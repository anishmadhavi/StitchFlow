...
| `App.tsx` | Line 112 | `const handleLogout = async () => {` |
| `App.tsx` | Line 119 | `const addUser = async (name: string, role: Role, mobile: str` |
| `App.tsx` | Line 127 | `// 2. Check for connection errors (e.g., wrong function name` |
| `App.tsx` | Line 134 | `// 3. Check for database errors returned INSIDE the function` |
| `App.tsx` | Line 145 | `const updateUser = async (userId: string, updates: Partial<U` |
| `App.tsx` | Line 159 | `const deleteUser = async (userId: string) => {` |
| `App.tsx` | Line 170 | `const createBatch = async (batchData: Partial<Batch>) => {` |
| `App.tsx` | Line 186 | `const finalizeCut = async (batchId: string, actualQty: SizeQ` |
| `App.tsx` | Line 199 | `const assignToKarigar = async (batchId: string, karigarId: s` |
| `App.tsx` | Line 235 | `const updateAssignmentStatus = async (batchId: string, assig` |
| `App.tsx` | Line 247 | `const handleQCSubmit = async (batchId: string, assignmentId:` |
| `App.tsx` | Line 281 | `const handleTransaction = async (userId: string, amount: num` |
| `App.tsx` | Line 301 | `const handleArchive = async (batchId: string) => {` |
